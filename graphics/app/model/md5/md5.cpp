...
namespace model
{
	namespace md5
	{
		...
		void strrmv(char * _szStr, const char * _szKeys)
		{
...
again:
		...
		ow::owBuffer * read_block_for_key(ow::owBuffer * _pBuffer, const char * _key)
		{
			...
			size_t n_read = _pBuffer->Read(pRet->GetBuffer(),pRet->Size() - 1);
		...
		bool ParseJoints(owBuffer * _pBuffer,MeshFilePtr _pMesh)
		{
			...
			for(size_t i = 0;i<_pMesh->m_nNumJoints;++i)
			{
		...
		void ClearMesh( MeshFilePtr _pMesh)
		{
			...
			for(size_t i = 0;i<_pMesh->m_nNumMeshes;++i)
			{
