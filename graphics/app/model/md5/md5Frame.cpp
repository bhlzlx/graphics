...
namespace Graphics
{
	...
	void InitMD5Model( MD5ModelPtr _pModel ,MD5FilePtr _pFile )
	{
		...
		for(uint32_t meshIdx = 0;meshIdx < _pFile->m_pMeshFile->m_nNumMeshes; ++meshIdx)
		{
			// 生成纹理
			...
			// 计算bindpose的顶点坐标
			for(uint32_t vertIdx = 0;vertIdx < nNumVertices ;++vertIdx)
			{
				// 累积 weight bias 计算顶点
...
void GetMD5KeyFrame(MD5FramePtr _pFrame,MD5FilePtr _pMD5File,uint32_t _mSeconds)
{
	...
	// 累积骨骼1 的变换
	for(size_t jointIndex = 1;jointIndex< _pMD5File->m_pMeshFile->m_nNumJoints;++jointIndex)
	{
		uint32_t parentJointIdx = jointMap[jointIndex].m_nParentIndex;
	...
	// 累积骨骼2 的变换
	for(uint32_t jointIndex = 1;jointIndex< _pMD5File->m_pMeshFile->m_nNumJoints;++jointIndex)
	{
		uint32_t parentJointIdx = jointMap[jointIndex].m_nParentIndex;
