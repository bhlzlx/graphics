...
namespace Graphics
{
	...
	void md5Model::Render( EffectOGL * _pEffect, glm::mat4 _model, uint8_t _wiredframe )
	{
		...
		for(uint32_t meshIdx = 0; meshIdx<this->m_nNumMeshes; ++meshIdx)
		{
	...
	{
		for(uint32_t animIdx = 0;animIdx < m_nNumAnimation; ++animIdx)
		{
		...
		for(uint32_t meshId = 0; meshId <m_nNumMeshes; ++meshId)
		{
